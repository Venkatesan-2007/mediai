                        ...
                        <div className="space-y-3">
                            ...
                            {showBookDropdown && filteredBooks.length > 0 && (
                                <div className="relative">
                                    <div className="absolute top-0 left-0 right-0 bg-white border-2 border-pink-200 rounded-xl shadow-lg max-h-64 overflow-y-auto z-20">
                                        {filteredBooks.map((book) => (
                                            <button
                                                key={book.id}
                                                onClick={() => handleSelectBook(book)}
                                                className="w-full text-left px-4 py-3 hover:bg-pink-50 border-b border-pink-100 last:border-b-0 transition-colors"
                                            >
                                                <div className="flex items-start justify-between">
                                                    ...
                                                    {selectedBook?.id === book.id && (
                                                        <svg className="w-5 h-5 text-pink-600 flex-shrink-0 ml-2" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            {selectedBook && (
                                <div className="p-4 bg-pink-50 border-2 border-pink-200 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center space-x-3 flex-1">
                                        <svg className="w-5 h-5 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                        </svg>
                                        <div>
                                            <p className="text-sm text-pink-600 font-medium">Selected Book</p>
                                            <p className="text-lg font-semibold text-pink-900">{selectedBook.title}</p>
                                        </div>
                                    </div>
                                    <button
                                        onClick={handleClearBook}
                                        className="text-pink-600 hover:text-pink-700 font-medium text-sm rounded-lg hover:bg-pink-100 px-3 py-2 transition-colors"
                                    >
                                        Change
                                    </button>
                                </div>
                    ...
                    {questions.length === 0 && !loading ? (
                        ...
                        <div className="space-y-4">
                            {questions.map((q, index) => (
                                <div
                                    key={index}
                                    className="p-5 bg-gradient-to-br from-pink-50 to-white rounded-xl border border-pink-100 hover:shadow-md transition-shadow"
                                >
                                    {/* Question Header */}
                                    <div className="flex items-start justify-between mb-3">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="text-pink-600 font-semibold text-xs bg-pink-100 px-3 py-1 rounded-full capitalize">
                                                {q.difficulty}
                                            </span>
                                        </div>
